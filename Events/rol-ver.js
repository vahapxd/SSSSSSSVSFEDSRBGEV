...
let komut = [
  ...
  {name: ["Rencia"], role: '839947536802512946',aliases: []}
...
if(["839091722716708897"].some(x => !message.member.roles.cache.has(x)) && !message.member.hasPermission("MANAGE_ROLES")) return permEmbed(message)
  ...
  if (!victim || message.mentions.members.size < 1 && isNaN(args[0])) return embedCreator("dsa",`${client.red} **Tüm argümanları doğru doldurunuz!** \`.${cmd.name} @Rencia/461212138346905600\` `,message,[true,15000])
  ...
  if (!victim.roles.cache.has(cmd.role)) {
    ...
    if(["839091722716708897"].some(x => !message.member.roles.cache.has(x)) && !message.member.hasPermission("MANAGE_ROLES")) return permEmbed(message)
    let victim = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
  if (!victim || message.mentions.members.size < 1 && isNaN(args[0])) return embedCreator("dsa",`${client.red} **Tüm argümanları doğru doldurunuz!** \`.${aliasess.name} @Rencia/461212138346905600\` `,message,[true,15000])
