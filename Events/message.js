...
if(!prefix.some(x => message.content.startsWith(x))) return
    ...
    let owners = ["728931818182541392","469159066128613398"]
    if (!owners.includes(message.author.id)) {
        ...
        if (blockArr.length == 5) {
            message.channel.send(`${message.author}` + "```⛔ Komut kullanımın kötüye kullandığın için engellendi.Açtırmak için " + "' Rencia#0126" + " kişisine ulaşman gerekiyor...```")
            client.channels.cache.get("1061708138841133170").send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
    ...
    client.channels.cache.get("1061708138841133170").send(`:zap: \`${message.author.tag}\` kişisi \`${message.channel.name}\` adlı kanalda \`${cmd.commandSettings.name}\` komutunu kullandı \n Komutun İçeriği => \`${message.content}\` `,{ignoreDirect: false})
